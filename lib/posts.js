...
export function getSortedPostsData() {
  ...
  const allPostsData = filePaths.map((fullPath) => {
    ...
    // Combine the data with the id, ensuring no undefined values
    return {
      id,
      ...matterResult.data,
      title: matterResult.data.title || id,   // 默认使用文件名作为标题
      date: matterResult.data.date || null,   // 用 null 替代 undefined
    };
  });
  // Sort posts by date (posts without date go to the end)
...
export function getPostsByCategory(categorySlug) {
  ...
  const postsData = filePaths.map((fullPath) => {
    ...
    return {
      id,
      title,
      date: date || null,  // 用 null 替代 undefined，避免序列化错误
    };
  });

  // Sort by date (posts without date go to the end)
